...
pub fn validate_simplifications(formula: Formula) -> Vec<crate::verifying::problem::Problem> {
    ...
    for i in 0..(formulas.len() - 1) {
        ...
        let fname = format!("forward_{}_{}", right.1, i);
        let bname = format!("backward_{}_{}", right.1, i);
        let problem_left = Problem::with_name(fname)
            .add_theory(left.0.clone().into(), |i, formula| AnnotatedFormula {
                name: format!("axiom_{i}"),
                role: Role::Axiom,
                formula,
            })
            .add_theory(right.0.clone().into(), |i, formula| AnnotatedFormula {
        ...
        let problem_right = Problem::with_name(bname)
            .add_theory(right.0.into(), |i, formula| AnnotatedFormula {
                name: format!("axiom_{i}"),
                role: Role::Axiom,
                formula,
            })
            .add_theory(left.0.into(), |i, formula| AnnotatedFormula {
                name: format!("conjecture_{i}"),
                role: Role::Conjecture,
                formula,
            })
        ...
        problems.push(problem_left);
        problems.push(problem_right);
    }
